...
namespace ChatApplication
{
    ...
    {
        public event EventHandler TextChangedInvoke;
        ...
        private void TextBoxULoad(object sender, EventArgs e)
        {
        ...
        {
            TextChangedInvoke?.Invoke(this,EventArgs.Empty);
        }
        ...
        {
            if (isCenterPlaceHolder == true&& (label.Location.X>0&& label.Location.Y>-3))
            {
                label.Location=new Point(label.Location.X-1, label.Location.Y-2);
            }
            else if(isCenterPlaceHolder == false&& (label.Location.X < placeholderlocation.X && label.Location.Y< placeholderlocation.Y)){
                label.Location = new Point(label.Location.X +1, label.Location.Y +2);
            }
            else
            {
